...
Protocols.IIIF = new Class({
  ...
  getTileURL: function(server,image,resolution,sds,contrast,k,x,y){
      ...
      var orig_x = x*f;
      var orig_y = y*f;
      var tile_x = this.tileSize.w * Math.pow(2,  this.num_resolutions - resolution - 1) ;
      var tile_y = this.tileSize.h * Math.pow(2,  this.num_resolutions - resolution - 1) ;
      var src = server + "/" + image + "/" + orig_x + "," + orig_y + "," + tile_x + "," + tile_y
      if (tile_x != 256) {
          src += "/!256,256";
      } else {
          src += "/full";
